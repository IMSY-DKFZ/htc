...
from htc_projects.species.tables import icg_table


def compute_icg_scores(run_dir: Path, recalculate: bool) -> None:
    ...
    if nested_index != 0:
        # We compute the ICG scores only for the first nested fold
        return

    if species == "human":
        # We do not have ICG data for humans
        return

    table_name = f"test_table_{species}_icg"
    ...
    df_selection = icg_table(label_mapping=mapping).query("species_name == @species")

    paths = DataPath.from_table(df_selection)
    assert len(paths) > 0, f"No paths found for species {species}"

    settings.log.info(f"Computing {species} ICG scores for the run {run_dir.name} ({len(paths)} ICG images)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute test scores for the ICG datasets.",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The timestamp to select runs for which ICG scores should be calculated.",
    )
    parser.add_argument(
        "--recalculate",
        default=False,
    ...
    for model_dir in sorted(settings.training_dir.iterdir()):
        ...
        for run_dir in sorted(model_dir.glob(f"{args.timestamp}*")):
            compute_icg_scores(run_dir, args.recalculate)
